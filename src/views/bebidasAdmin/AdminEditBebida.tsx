...
export default function AdminEditBebida() {

    const { bebida, actualizarBebida, fetchBebida } = useAppStore();

    const { id } = useParams();
    const bebidaId = Number(id);
    ...
    const { register, handleSubmit, reset, formState: { errors }} = useForm<BebidaFormValues>({
        defaultValues: bebida
    });
    ...
    const editBebida = async(data:BebidaFormValues) => {
        try {
        await actualizarBebida(data, bebidaId)
  ...
  return (
    <div className=' container mx-auto'>
        <h2 className=' text-center font-bold text-4xl mt-8'>Editar Bebida</h2>

        <form onSubmit={handleSubmit(editBebida)} className=' mt-8 space-y-4 bg-slate-200 p-4 rounded-lg'>
            <div className=' '>
            ...
            <div className=" overflow-hidden">
                <input type="submit" value="Actualizar producto" className=" bg-gray-700 w-full p-2 rounded-lg text-white font-bold cursor-pointer hover:bg-gray-800 hover:scale-105 duration-200"/>
